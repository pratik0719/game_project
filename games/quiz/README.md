...
## 📌 Overview
- **What the game is:** Answer correctly and quickly for max score.
- **Game category:** Casual
- **Accent color:** `#ffb703`
- **Theme:** Yellow electric quiz panel

## 🕹️ How to Play
### Step by Step
1. Open **Quiz Reactor** from the home screen.
...
| Control | Action |
|---|---|
| `Mouse / Tap` | Select options |
| `Restart` | Replay quiz |

### Objective
- Answer correctly and quickly for max score.

## 🧠 Game Logic
### Core Algorithm Used
- Questions and options are loaded from XML at runtime.
- Per-question countdown enforces fast decision-making.
- Fast answers get a score multiplier via remaining-time bonus.
- Correct option highlighting and lockout prevent double submits.
...
|---|---|---|
| `timer_per_question` | `value` | Config value `timer_per_question`. |
| `points_per_correct` | `value` | Config value `points_per_correct`. |
| `questions/question` | `value` | Nested value under `questions`. |
| `questions/question` | `value` | Nested value under `questions`. |
| `questions/question` | `value` | Nested value under `questions`. |
| `questions/question` | `value` | Nested value under `questions`. |
| `questions/question` | `value` | Nested value under `questions`. |
...
```xml
﻿<quiz>
  <timer_per_question>15</timer_per_question>
  <points_per_correct>10</points_per_correct>
  <questions>
    <question>
    ...
    <question>
      ...
      <option>Central Style Sheets</option>
    ...
    <question>
      <text>Which language runs in a web browser?</text>
    ...
    <question>
      <text>Which company created Flask?</text>
...
| File | Description |
|---|---|
| `games/quiz/quiz.js` | Main frontend game logic. |
| `games/quiz/quiz.xml` | Game configuration values. |
| `games/quiz/README.md` | Game documentation for developers. |

## 🔗 API Endpoints Used
| Method | Route | Purpose |
|---|---|---|
| `GET` | `/api/config/quiz` | Load game settings from XML as JSON. |
| `POST` | `/api/score` | Save new score entry to `scores.xml`. |
| `GET` | `/api/leaderboard` | Read top scores (game-specific filtering client-side). |
| `GET` | `/game/quiz` | Open game container page. |
...
- Game posts score with `game="quiz"` to `/api/score`.
