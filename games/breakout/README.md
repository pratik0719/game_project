...
## 📌 Overview
- **What the game is:** Break bricks, preserve lives, and reach high levels.
- **Game category:** Arcade
- **Accent color:** `#e879f9`
- **Theme:** Fuchsia neon brick field

## 🕹️ How to Play
### Step by Step
1. Open **Breakout Neon** from the home screen.
...
### Objective
- Break bricks, preserve lives, and reach high levels.

## 🧠 Game Logic
### Core Algorithm Used
- Ball reflection computes response from wall/paddle impact angles.
- Brick destruction uses overlap collision detection.
- Level progression rebuilds brick field and increases speed after clear.
- Lives decrement on miss; game ends at zero lives.
...
|---|---|---|
| `rows` | `value` | Config value `rows`. |
| `cols` | `value` | Config value `cols`. |
| `ball_speed` | `value` | Config value `ball_speed`. |
| `paddle_width` | `value` | Config value `paddle_width`. |
...
﻿<breakout>
  <rows>5</rows>
  <cols>9</cols>
  <ball_speed>4.2</ball_speed>
  <paddle_width>88</paddle_width>
</breakout>
```
...
| File | Description |
|---|---|
| `games/breakout/breakout.js` | Main frontend game logic. |
| `games/breakout/breakout.xml` | Game configuration values. |
| `games/breakout/README.md` | Game documentation for developers. |

## 🔗 API Endpoints Used
| Method | Route | Purpose |
|---|---|---|
| `GET` | `/api/config/breakout` | Load game settings from XML as JSON. |
| `POST` | `/api/score` | Save new score entry to `scores.xml`. |
| `GET` | `/api/leaderboard` | Read top scores (game-specific filtering client-side). |
| `GET` | `/game/breakout` | Open game container page. |
...
- Game posts score with `game="breakout"` to `/api/score`.
