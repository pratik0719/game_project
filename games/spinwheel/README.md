...
## 📌 Overview
- **What the game is:** Stack the highest total prize score from repeated spins.
- **Game category:** Casual
- **Accent color:** `#c084fc`
- **Theme:** Purple prize wheel

## 🕹️ How to Play
### Step by Step
1. Open **Spin the Wheel** from the home screen.
...
| Control | Action |
|---|---|
| `SPIN` | Start wheel animation |
| `Save Total Score` | Submit cumulative score |
| `Play Again` | Reset totals |

### Objective
- Stack the highest total prize score from repeated spins.

## 🧠 Game Logic
### Core Algorithm Used
- Winning segment is computed from normalized final rotation + pointer angle.
- Easing function decelerates spin smoothly using cubic ease-out.
- Segment probability is equal when segments have equal angular size.
- Result updates in centered panel with color-matched highlight.
...
|---|---|---|
| `spin_duration` | `value` | Config value `spin_duration`. |
| `segments/segment@label` | `attribute` | Nested attribute under `segments`. |
| `segments/segment@color` | `attribute` | Nested attribute under `segments`. |
| `segments/segment@prize` | `attribute` | Nested attribute under `segments`. |
| `segments/segment@label` | `attribute` | Nested attribute under `segments`. |
| `segments/segment@color` | `attribute` | Nested attribute under `segments`. |
| `segments/segment@prize` | `attribute` | Nested attribute under `segments`. |
| `segments/segment@label` | `attribute` | Nested attribute under `segments`. |
...
```xml
﻿<spinwheel>
  <spin_duration>4600</spin_duration>
  <segments>
    <segment label="Bronze" color="#f97316" prize="40" />
    <segment label="Silver" color="#60a5fa" prize="60" />
    <segment label="Golden" color="#facc15" prize="90" />
    <segment label="Mystery" color="#c084fc" prize="110" />
    <segment label="Turbo" color="#22d3ee" prize="75" />
    <segment label="Lucky" color="#34d399" prize="50" />
    <segment label="Mega" color="#fb7185" prize="120" />
    <segment label="Bonus" color="#a3e635" prize="80" />
  </segments>
</spinwheel>
```
...
| File | Description |
|---|---|
| `games/spinwheel/spinwheel.js` | Main frontend game logic. |
| `games/spinwheel/spinwheel.xml` | Game configuration values. |
| `games/spinwheel/README.md` | Game documentation for developers. |

## 🔗 API Endpoints Used
| Method | Route | Purpose |
|---|---|---|
| `GET` | `/api/config/spinwheel` | Load game settings from XML as JSON. |
| `POST` | `/api/score` | Save new score entry to `scores.xml`. |
| `GET` | `/api/leaderboard` | Read top scores (game-specific filtering client-side). |
| `GET` | `/game/spinwheel` | Open game container page. |
...
- Game posts score with `game="spinwheel"` to `/api/score`.
