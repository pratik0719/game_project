...
## 📌 Overview
- **What the game is:** Create three in a row before the opponent.
- **Game category:** Board
- **Accent color:** `#ff4d9d`
- **Theme:** Magenta neon 3x3 board

## 🕹️ How to Play
### Step by Step
1. Open **Tic Tac Toe Grid** from the home screen.
...
| Control | Action |
|---|---|
| `Mouse / Tap` | Place marks |
| `Mode buttons` | Switch PvP/AI |
| `Replay` | Reset board |

### Objective
- Create three in a row before the opponent.

## 🧠 Game Logic
### Core Algorithm Used
- Win checks evaluate rows, columns, and diagonals every move.
- AI move selection uses easy/medium/hard heuristics.
- Draw detection triggers when board fills without a winner.
- XML controls default AI mode and score values.
...
|---|---|---|
| `board_size` | `value` | Config value `board_size`. |
| `ai_default` | `value` | Config value `ai_default`. |
| `ai_difficulty` | `value` | Config value `ai_difficulty`. |
| `win_score` | `value` | Config value `win_score`. |
| `draw_score` | `value` | Config value `draw_score`. |
| `loss_score` | `value` | Config value `loss_score`. |

### Default XML Values
```xml
﻿<tictactoe>
  <board_size>3</board_size>
  <ai_default>true</ai_default>
  <ai_difficulty>medium</ai_difficulty>
  <win_score>100</win_score>
  <draw_score>50</draw_score>
  <loss_score>15</loss_score>
</tictactoe>
```
...
| File | Description |
|---|---|
| `games/tictactoe/tictactoe.js` | Main frontend game logic. |
| `games/tictactoe/tictactoe.xml` | Game configuration values. |
| `games/tictactoe/README.md` | Game documentation for developers. |

## 🔗 API Endpoints Used
| Method | Route | Purpose |
|---|---|---|
| `GET` | `/api/config/tictactoe` | Load game settings from XML as JSON. |
| `POST` | `/api/score` | Save new score entry to `scores.xml`. |
| `GET` | `/api/leaderboard` | Read top scores (game-specific filtering client-side). |
| `GET` | `/game/tictactoe` | Open game container page. |
...
- Game posts score with `game="tictactoe"` to `/api/score`.
