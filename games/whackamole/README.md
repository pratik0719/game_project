...
## 📌 Overview
- **What the game is:** Hit as many moles as possible before time runs out.
- **Game category:** Arcade
- **Accent color:** `#4ade80`
- **Theme:** Lime reaction arena

## 🕹️ How to Play
### Step by Step
1. Open **Whack-a-Mole** from the home screen.
...
## 🧠 Game Logic
### Core Algorithm Used
- Random hole selection chooses the next active mole location each cycle.
- Mole visibility uses timed show/hide windows.
- Speed scales over time via XML interval levels by elapsed second thresholds.
- Final score is total successful hits before countdown ends.
...
|---|---|---|
| `mole_count` | `value` | Config value `mole_count`. |
| `game_duration` | `value` | Config value `game_duration`. |
| `speed_levels/level@second` | `attribute` | Nested attribute under `speed_levels`. |
| `speed_levels/level@interval` | `attribute` | Nested attribute under `speed_levels`. |
| `speed_levels/level@second` | `attribute` | Nested attribute under `speed_levels`. |
| `speed_levels/level@interval` | `attribute` | Nested attribute under `speed_levels`. |
| `speed_levels/level@second` | `attribute` | Nested attribute under `speed_levels`. |
| `speed_levels/level@interval` | `attribute` | Nested attribute under `speed_levels`. |
...
```xml
﻿<whackamole>
  <mole_count>9</mole_count>
  <game_duration>30</game_duration>
  <speed_levels>
    <level second="0" interval="900" />
    <level second="10" interval="650" />
    <level second="20" interval="430" />
  </speed_levels>
</whackamole>
```
...
| File | Description |
|---|---|
| `games/whackamole/whackamole.js` | Main frontend game logic. |
| `games/whackamole/whackamole.xml` | Game configuration values. |
| `games/whackamole/README.md` | Game documentation for developers. |

## 🔗 API Endpoints Used
| Method | Route | Purpose |
|---|---|---|
| `GET` | `/api/config/whackamole` | Load game settings from XML as JSON. |
| `POST` | `/api/score` | Save new score entry to `scores.xml`. |
| `GET` | `/api/leaderboard` | Read top scores (game-specific filtering client-side). |
| `GET` | `/game/whackamole` | Open game container page. |
...
- Game posts score with `game="whackamole"` to `/api/score`.
