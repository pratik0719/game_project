...
## 📌 Overview
- **What the game is:** Pass as many pipes as possible without collisions.
- **Game category:** Arcade
- **Accent color:** `#38bdf8`
- **Theme:** Sky-blue side scroller

## 🕹️ How to Play
### Step by Step
1. Open **Flappy Burst** from the home screen.
...
### Objective
- Pass as many pipes as possible without collisions.

## 🧠 Game Logic
### Core Algorithm Used
- Physics integrates gravity and velocity every frame (`vy += g`, `y += vy`).
- Pipe generation is procedural with random gap center inside safe bounds.
- Collision checks include ground/ceiling and pipe rectangles.
- Score increments when bird passes uncounted pipes.
...
|---|---|---|
| `gravity` | `value` | Config value `gravity`. |
| `pipe_gap` | `value` | Config value `pipe_gap`. |
| `pipe_speed` | `value` | Config value `pipe_speed`. |
| `bird_color` | `value` | Config value `bird_color`. |

### Default XML Values
```xml
﻿<flappy>
  <gravity>0.38</gravity>
  <pipe_gap>132</pipe_gap>
  <pipe_speed>2.4</pipe_speed>
  <bird_color>#38bdf8</bird_color>
</flappy>
```
...
| File | Description |
|---|---|
| `games/flappy/flappy.js` | Main frontend game logic. |
| `games/flappy/flappy.xml` | Game configuration values. |
| `games/flappy/README.md` | Game documentation for developers. |

## 🔗 API Endpoints Used
| Method | Route | Purpose |
|---|---|---|
| `GET` | `/api/config/flappy` | Load game settings from XML as JSON. |
| `POST` | `/api/score` | Save new score entry to `scores.xml`. |
| `GET` | `/api/leaderboard` | Read top scores (game-specific filtering client-side). |
| `GET` | `/game/flappy` | Open game container page. |
...
- Game posts score with `game="flappy"` to `/api/score`.
